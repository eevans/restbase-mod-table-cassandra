...
class RevisionPolicyManager {
    ...
    _rangeDelete(item) {
        ...
        req.schema.iKeys.forEach((attr) => {
            if (attr !== 'tid') {
                predicates[attr] = req.query.attributes[attr];
            }
        });
        predicates.tid = { lt: delTid.toString() };

        const condition = dbu.buildCondition(predicates, req.schema, true);
        const query = `DELETE FROM ${keyspace}.${columnfamily} WHERE ${condition.cql}`;
        const queryOpts = { consistency: req.consistency, prepare: true };

        this.db.log('debug/table/cassandra/renderRetentionRangeDelete', {
            query,
            params: condition.params,
            msg: 'Deleting renders older than TTL'
        });

        return this.db.client.execute(query, condition.params, queryOpts)
        .catch((e) => {
            this.db.log('error/table/cassandra/renderRetentionRangeDelete', e);
        });
    }
    ...
    handleRow(row) {
        ...
        if (this.policy.type === 'latest_hash') {
            this.noop = true;    // This need run only once.
            return this._rangeDelete(row);
        }
