...
class DB {
    ...
    _deleteOlderContent(req) {
        // XXX: If the table has a revision, do a range delete based on that revision.
        if (req.schema.attributes.rev) {
            return this._deleteOlderRange(req);
        }
    ...
    _deleteOlderRange(req) {
        // FIXME: Do not hard-code probability
        // XXX: This will provide different results for different logical tables
        // (parsoid.html, parsoid.data-parsoid, etc); Problem?
        if (Math.random() < 0.1) {
            return P.resolve();
        }

        // FIXME: Obviously it's unacceptable to go all ninja on the Cassandra timeline
        // table directly; This is just for prototyping purposes, a proper abstraction for
        // timeline access will be needed.
        const reqTid = req.query.attributes.tid;
        const delTid = TimeUuid.fromDate(new Date(reqTid.getDate().getTime() - 3600000));

        const timelineCql =
            `SELECT value FROM ${dbu.cassID('local_group_default_T_timeline')}.${dbu.cassID('data')}
             WHERE "_domain" = 'internal' AND key = ? AND tid < ? LIMIT 1`;

        this.client.execute(timelineCql, [req.domain, delTid.toString()], { prepare: true })
        .then((res) => {
            if (res.rows.length !== 1) {
                this.log(
                    'warn/table/cassandra/revisionRetention',
                    {
                        msg: 'No timeline results returned',
                        domain: req.domain,
                        tid: delTid.toString()
                    }
                );
            ...
            const query = `DELETE FROM ${dbu.cassID(req.keyspace)}.${dbu.cassID(req.columnfamily)}`;
            const predicates = { "_domain": req.domain };

            // Add the hash keys
            req.schema.iKeys.forEach((att) => {
                if (req.schema.iKeyMap[att].type === 'hash') {
                    predicates[att] = req.query.attributes[att];
                }
            });

            // We wouldn't be here if there weren't a `rev` attribute...
            predicates.rev = { lt: row.value };
            const condition = dbu.buildCondition(predicates, req.schema, true);

            this.log('debug/table/cassandra/revisionRetentionRangeDelete', {
                query: `${query} WHERE ${condition.cql}`,
                params: condition.params,
                msg: 'Deleting revisions older than TTL'
            });

            // Delete
            return this.client.execute(`${query} WHERE ${condition.cql}`, condition.params, {
                prepare: true
            })
            .catch((e) => {
                this.log('error/table/cassandra/revisionRetentionRangeDelete', e);
            });
        });
    }
